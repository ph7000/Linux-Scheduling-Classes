...
restart:
	...
	for_each_active_class(class) {
		if (class == &oven_sched_class) {
			// printk(KERN_INFO "oven class here\n");
		}
...
const struct sched_class *__setscheduler_class(int policy, int prio)
{
	...
	if (policy == 8) {
		printk(KERN_INFO "OVEN: setting sched class to oven\n");
		return &oven_sched_class;
	}
...
void __init sched_init(void)
{
	unsigned long ptr = 0;
	int i;
	printk(KERN_ALERT "OVEN: sched_init entry\n");
	...
	BUG_ON(!sched_class_above(&rt_sched_class, &oven_sched_class));
	BUG_ON(!sched_class_above(&oven_sched_class, &fair_sched_class));
	BUG_ON(!sched_class_above(&fair_sched_class, &idle_sched_class));
#ifdef CONFIG_SCHED_CLASS_EXT
	BUG_ON(!sched_class_above(&oven_sched_class, &ext_sched_class));
